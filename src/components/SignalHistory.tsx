    ...
    <section className="signal-history" aria-label="История торговых сигналов">
      ...
      <div className="signal-list">
        {signals.map((signal) => <button key={signal.id} className={`signal-card ${signal.side} ${selected?.id === signal.id ? 'selected' : ''}`} onClick={() => { setSelected(signal); onSelectSymbol(signal.symbol) }}>
          <span className="signal-card-main"><b>{signal.symbol.replace('USDT', '')} · {signal.side.toUpperCase()}</b><small>{formatTimestamp(signal.detectedAt)}</small></span>
      ...
      {selected && <article className="signal-snapshot">
        <header><div><span className="eyebrow">FIXED SNAPSHOT</span><strong>{selected.symbol} · {selected.side.toUpperCase()} · {formatTimestamp(selected.detectedAt)}</strong></div><button onClick={() => setSelected(null)} aria-label="Закрыть снимок">×</button></header>
