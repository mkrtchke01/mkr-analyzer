...
export function evaluateSignalCandle(signal: ManagedSignal, candle: Candle): SignalCandleOutcome {
  ...
  const hitTp2 = signal.status !== 'tp2' && reached(signal.side, candle, signal.tp2Price, 'target')
  const hitTp3 = signal.tp3Price !== undefined && reached(signal.side, candle, signal.tp3Price, 'target')
  const tp1Share = signal.tp3Price === undefined ? 0.5 : 0.34
  ...
  const priorOutcome = signal.status === 'active' ? 0 : signal.status === 'tp1' ? signal.tp1RiskMultiple * tp1Share : signal.tp1RiskMultiple * tp1Share + signal.tp2RiskMultiple * tp2Share

  if (hitStop && (hitTp1 || hitTp2 || hitTp3)) return { type: 'ambiguous' }
  if (hitTp3) return { type: 'tp3', outcomeR: signal.tp1RiskMultiple * tp1Share + signal.tp2RiskMultiple * tp2Share + tp3RiskMultiple * 0.33 }
  if (hitTp2) {
    const outcomeR = (signal.status === 'active' ? signal.tp1RiskMultiple * tp1Share : priorOutcome) + signal.tp2RiskMultiple * tp2Share
    return signal.tp3Price === undefined ? { type: 'tp2', outcomeR } : { type: 'tp2', nextStopPrice: signal.entryPrice, outcomeR }
  }
  if (hitTp1) return { type: 'tp1', nextStopPrice: signal.entryPrice, outcomeR: signal.tp1RiskMultiple * tp1Share }
