...
export function createSignalSnapshot(symbol: string, candles: Candle[], plan: TradePlan, detectedAt: string): string {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><rect width="100%" height="100%" fill="#0c1019"/><text x="${left}" y="31" fill="#edf1f7" font-family="Arial, sans-serif" font-size="20" font-weight="700">${escapeXml(symbol)} · SETUP ${plan.stop.side.toUpperCase()}</text><text x="${left}" y="53" fill="#8390a3" font-family="monospace" font-size="11">${escapeXml(detectedAt)} · 5m snapshot</text>${grid}${bodies}${stop}${targets}<text x="${left}" y="${height - 18}" fill="#748095" font-family="monospace" font-size="10">Snapshot is fixed at signal confirmation</text></svg>`
}
