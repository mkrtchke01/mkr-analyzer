...
export function createSignalSnapshot(symbol: string, candles: Candle[], plan: TradePlan, detectedAt: string): string {
  ...
  const referenceLevels = [
    ...(plan.triggerLevel ? [{ ...plan.triggerLevel, color: '#f2c15d' }] : []),
    ...(plan.chartLevels ?? []),
  ...
  const levels = [plan.stop.price, ...referenceLevels.map((reference) => reference.price), ...plan.takeProfits.map((target) => target.price)].filter((price): price is number => price !== undefined)
  ...
  const referenceLines = referenceLevels.map((reference) => level(reference.label, reference.price, reference.color ?? '#f2c15d')).join('')
  const targets = plan.takeProfits.map((target) => level(`${target.id} ${target.price.toPrecision(6)}`, target.price, '#31d28c')).join('')

  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><rect width="100%" height="100%" fill="#0c1019"/><text x="${left}" y="31" fill="#edf1f7" font-family="Arial, sans-serif" font-size="20" font-weight="700">${escapeXml(symbol)} · ${escapeXml(plan.setupName)} · ${plan.stop.side.toUpperCase()}</text><text x="${left}" y="53" fill="#8390a3" font-family="monospace" font-size="11">${escapeXml(detectedAt)} · 5m snapshot</text>${grid}${bodies}${referenceLines}${stop}${targets}<text x="${left}" y="${height - 18}" fill="#748095" font-family="monospace" font-size="10">Snapshot is fixed at signal confirmation</text></svg>`
}
